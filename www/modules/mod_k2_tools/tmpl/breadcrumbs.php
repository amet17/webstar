...
<div id="k2ModuleBox<?php echo $module->id; ?>" class="k2BreadcrumbsBlock <?php echo $params->get('moduleclass_sfx'); ?>">
	...
	if ($params->get('home')) {
		...
		$output .= '<noindex><a rel="nofollow" href="'.JURI::root().'">'.$params->get('home',JText::_('Home')).'</a></noindex>';
		if (count($path)) {
			foreach ($path as $link) {
				$output .= '<span class="bcSeparator">'.$params->get('seperator','&raquo;').'</span>' .'<noindex>'.$link .'</noindex>';
			}
		}
		if($title){
