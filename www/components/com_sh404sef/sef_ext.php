 ...
 * @version     $Id: sef_ext.php 1827 2011-02-25 09:53:34Z silianacom-svn $
 */
...
class sef_404
{
  ...
  function revert(&$url_array, $pos)
  {
    ...
    // read from db
    $sql="SELECT oldurl, newurl FROM #__redirection WHERE oldurl = ".$database->Quote($req)." ORDER BY rank ASC LIMIT 1"; // V 1.2.4.q
    ...
    if ($row) {
      ...
      // update the count
      $database->setQuery("UPDATE #__redirection SET cpt=(cpt+1) WHERE `newurl` = ".$database->Quote($row->newurl)
      ...
      if ($realloc) {
        ...
        do {
          ...
          if ($temploc != '') {
            ...
            $sql =  "SELECT id, newurl, rank, dateadd FROM #__redirection WHERE oldurl = ".$database->Quote($temploc)
