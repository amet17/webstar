...
var cachecleaner_load = function( id, editorname )
{
	...
	var myXHR = new XHR( {
			...
			onSuccess: function( data ) {
				...
				if( data.length > 100 ) {
					classname = 'failure';
					$( 'cachecleaner_msg' ).setHTML( cachecleaner_msg_inactive ).addClass( classname );
					cachecleaner_show_end( 4000 );
				} else {
					if ( data == cachecleaner_msg_success ) {
						classname = 'success';
					}
					$( 'cachecleaner_msg' ).setHTML( data ).addClass( classname );
					cachecleaner_show_end( 2000 );
				}
			},
			onFailure: function() {
				classname = 'failure';
				$( 'cachecleaner_msg' ).setHTML( cachecleaner_msg_failure ).addClass( classname );
				cachecleaner_show_end( 2000 );
			}
		} );
