 ...
 * @version     $Id: sh404sef.class.php 1933 2011-05-21 07:06:17Z silianacom-svn $
 */
...
function shFinalizeURL( $url) {
  $sefConfig = shRouter::shGetConfig();
  if ($sefConfig->shEncodeUrl && !empty($url) && (strpos($url, '/index.php?/') === false)) {  // V w 27/08/2007 13:38:34 sh_NetURL does not work if
...
function shAddSefUrlToDBAndCache( $nonSefUrl, $sefString, $rank, $urlType) {
  ...
  if ($urlType == sh404SEF_URLTYPE_AUTO) {  // before adding a full sef, we must check it does not already exists as a 404
    $query = 'SELECT id, newurl FROM #__redirection where oldurl='.$database->Quote($sefString)
    ...
    if (!empty($result)) {
      ...
      $query = 'select id, newurl, rank from #__redirection where oldurl='.$database->Quote($sefString).' AND newurl <> \''.addslashes(urldecode($nonSefUrl)).'\' order by ' . $database->nameQuote( 'rank') . ' desc';
      $database->setQuery($query);
...
function shGetIsoCodeFromName($langName) {
  ...
  global $shMosConfig_lang, $shMosConfig_locale, $shMosConfig_shortcode;
...
function shGetNameFromIsoCode($langCode) {
  ...
  global $shMosConfig_lang, $shMosConfig_locale, $shLangNameCache;
...
function shIsHomepage( $string) {
  ...
  if( !isset( $pages[$string])) {
    ...
    $pages[$string] = shSortUrl(shCleanUpLangAndPag($shTempString)) == $home; // version t added sorting
  }
  return $pages[$string];
}
...
function shShouldRedirectFromNonSef( $shPageInfo) {
  ...
  $shouldRedirect = $shouldRedirect && empty($shPageInfo->autoRedirectsDisabled) && $sefConfig->shRedirectNonSefToSef
