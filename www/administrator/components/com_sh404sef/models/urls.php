 ...
 * @version     $Id: urls.php 1974 2011-05-30 11:12:03Z silianacom-svn $
 */
...
class Sh404sefModelUrls extends Sh404sefClassBaselistmodel {
  ...
  public function purge( $type = 'auto') {
    ...
    $nameQuoted = array( 'id', '#__redirection', 'dateadd', 'rank', 'oldurl');
    $quoted = array( '0000-00-00');
    try {
      // select custom with lowest rank
      $customs = Sh404sefHelperDb::queryQuote($query, $nameQuoted, $quoted)->loadResultArray();
      // make sure there is at least one of those with rank = 0
      Sh404sefHelperDb::updateIn( '#__redirection', array( 'rank' => 0), 'id', $customs);
  ...
  protected function _buildListSelect( $options) {
    ...
    if ($simpleUrlList) {
      ...
      if ($getPageId) {
        ...
        $select[] = 'from ' . $this->_db->nameQuote( '#__sh404SEF_meta') . ' as m';
      } else {
        $select[] = 'from ' . $this->_db->nameQuote( '#__redirection') . ' as u1';
      }
    } else {
  ...
  protected function _buildListJoin( $options) {
    ...
    if ($simpleUrlList) {
      if ($getPageId) {
        $join[] = 'left join ' . $this->_db->nameQuote( '#__redirection') . ' as u1 on p.newurl=u1.newurl';
      } else if( $getMetaData && $layout == 'export') {
        $join[] = 'left join ' . $this->_db->nameQuote( '#__redirection') . ' as u1 on m.newurl=u1.newurl';
      } else if( $getMetaData) {
