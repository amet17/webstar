...
var nnManager = new Class({
	...
	set_local_data: function( id, alias, data, load_external )
	{
		...
		if ( data === 0 ) {
			...
			document.getElement( '#version_'+id+'_version' ).setText( this.versions[alias] ).setStyle( 'display', 'block' );
	...
	load_external_data: function( id, alias, version )
	{
		...
		var url = 'http://www.nonumber.nl/ext/extension.php?ext='+id+'&version='+version;
	...
	set_external_data: function( id, alias, data )
	{
		...
		if ( data != 0 && data != '' ) {
			...
			document.getElements( '#changelog_'+id+'_success span' )
				.setProperty( 'title', NNEM_CHANGELOG+'::'+changelog );
	...
	get_langs: function( id, data )
	{
		...
		var url = 'http://www.nonumber.nl/ext/extension.php?ext='+id+data;
	...
	set_lang_state: function( id, data )
	{
		...
		for ( i=0; i<langs.length; i++ ) {
			...
			if ( version_state != 'older' && version_state != 'same' ) {
				...
				}).setHTML( lang+' ('+new_version+')' ).inject( new Element( 'li' ).inject( list ) ) ;
			}
		}

		if ( hasLangs ) {
			document.getElement( '#nonumbermanager_langs_title' ).setStyle( 'display', 'block' );
			document.getElement( '#version_button_'+id+'_langs' ).empty().adopt( list ).setStyle( 'display', 'block' );
		}
	},
	...
	load_license: function( id )
	{
		...
		if ( field.value ) {
			...
			var url = 'http://www.nonumber.nl/ext/license.php?host='+host.value+'&code='+field.value+'&ext='+id;
