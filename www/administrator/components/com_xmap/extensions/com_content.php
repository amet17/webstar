...
/**
 * $Id: com_content.php 119 2010-06-26 11:49:59Z guilleva $
 * $LastChangedDate: 2010-06-26 06:49:59 -0500 (Sat, 26 Jun 2010) $
        ...
        switch ($view) {
            ...
            case 'article':
                ...
                if ($id) {
                    ...
                    $db = & JFactory::getDBO();
                    $db->setQuery(
                            "SELECT UNIX_TIMESTAMP(modified) as modified, "
                    ...
                    if ( $params['max_images'] ) {
                        $node->images = xmap_com_content::getImages($item->introtext . $item->fulltext,$params['max_images']);
                    }
                    $node->expandible = false;
                }
            case 'archive':
...
//			$params['max_art_age'] = 2;
        ...
        if (count($items) > 0) {
            ...
            foreach ($items as $item) {
                ...
                if ($params['add_images']) {
                    $node->images = xmap_com_content::getImages($text,$params['max_images']);
                }

                if ($params['add_pagebreaks']) {
                    $subnodes = xmap_com_content::getPagebreaks($text,$node->link);
    ...
    function getPagebreaks($text,$baseLink)
    {
