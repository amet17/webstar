...
var AdminBarDocker = new Class({
	...
	initialize: function( template, texts, hidetopbar, ignorecookie )
	{
		...
		var self = this;
		this.bodyElement = document.id( document.body );
		...
		this.docks.top_slide = new Element( 'div' ).inject( this.docks.top_inner );
	...
	toggleDockState: function() {
		if ( this.dock_state == 'docked' ) {
			...
			if ( this.hidetopbar && document.id( 'border-top' ) ) {
				document.id( 'border-top' ).setStyle( 'display', 'none' );
				if ( document.id( 'content-box' ) ) {
					document.id( 'content-box' ).setStyle( 'border-top', '1px solid #CCCCCC' );
				}
			...
			if ( this.hidetopbar && document.id( 'border-top' ) ) {
				document.id( 'border-top' ).setStyle( 'display', '' );
				if ( document.id( 'content-box' ) ) {
					document.id( 'content-box' ).setStyle( 'border-top', 'none' );
				}
			}
		}
	...
	setElementsPositions : function() {
		...
		this.elements.all.each ( function( el, i ){
			dummy = document.id( el.getProperty( 'rel' ) );
			if ( i == 0 ) {
