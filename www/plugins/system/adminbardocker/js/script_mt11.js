...
var AdminBarDocker = new Class({
	...
	initialize: function( template, texts, hidetopbar, ignorecookie )
	{
		...
		this.docks.top_slide = new Element( 'div' ).injectInside(
			new Element( 'div', { 'class': 'abd_dock_slide' } ).injectInside( this.docks.top_inner )
		);
	...
	initState: function()
	{
		...
		if ( ( this.ignorecookie && this.dock_state == 'undocked' ) || ( !this.ignorecookie && Cookie.get( 'abd_dock_state' ) == 'undocked' ) ) {
