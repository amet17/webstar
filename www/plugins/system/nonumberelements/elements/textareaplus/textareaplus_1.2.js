/**
 * JavaScript file for Element: Text Area Plus (MooTools 1.2 compatible)
 *
...
TextAreaResizer = new Class({
	...
	build: function()
	{
		...
		document.id(handle).setStyle( 'width', this.element.getStyle( 'width' ) );
		this.element.makeResizable({
			...
			onStart: function(el){
				autoHeightParents( handle );
				document.id(document.body).addClass('dragging');
		    },
			onComplete: function(el){
		    	document.id(document.body).removeClass('dragging');
		    },
		    onDrag: function(el){
				document.id(handle).setStyle( 'width', this.element.getStyle( 'width' ) );
		    }
