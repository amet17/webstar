	...
	class plgCommunityWalls extends CApplications
	{
		...
		function onActivityContentDisplay( $args )
		{
			...
			if( CPrivacy::isAccessAllowed($my->id, $args->target, 'user', 'privacyProfileView') )
			{
				...
				$videoContent	= '';
				$params			= new JParameter( $args->params );
				...
				if( !$videoLink )
				{
					...
					$tmpl->set( 'comment' , JString::substr($html, 0, $config->getInt('streamcontentlength') ) );
				...
				else
				{
					...
					$html 			.= '<!-- avatar --><div class="avatarWrap"><a href="javascript:void(0);" onclick="joms.activities.showVideo(\'' . $args->id . '\');"><img width="64" src="'. $image . '" class="avatar"/></a></div><!-- avatar -->';
					$videoPlayer	= $provider->getViewHTML( $provider->getId() , '300' , '300' );
					$comment		= JString::str_ireplace( $videoLink , '' , $comment );
