...
class plgEditorRokPad extends JPlugin
{
	...
	function onSave($editor) {
		...
		$cleanName = preg_replace("/[\[\]-_]/", "", $editor);
	...
	function _loadFormatting($editorname) 
	{
		...
		$cleanName = preg_replace("/[\[\]-_]/", "", $editorname);
		...
		if (file_exists($buttonsXML) && is_readable($buttonsXML) && $this->params->get('rokpad-show-formatter')) {
			...
			foreach($buttons as $button) {
				if ($button->name() == 'button') {
					...
					$jsLoaders .= "$('".$cleanName."').getParent().getElement('div.".$name."').rokpadname = '".$name."'; window.rokeditors['".$cleanName."'].attachButton($('".$cleanName."').getParent().getElement('div.".$name."'));";
